...
public class DbAccessor
{
	...
	public static void Insert<T>(IDbConnection connection, T instance, int Timeout = 30)
	{
		...
		foreach (var idnetifer in def.ChildIdentifers)
		{
			...
			{
				var insertMethod = typeof(DbAccessor).GetMethod(nameof(DbAccessor.Insert))!.MakeGenericMethod(children.GenericType);
	...
	public static void Update<T>(IDbConnection connection, T instance, int Timeout = 30)
	{
		...
		foreach (var idnetifer in def.ChildIdentifers)
		{
			...
			foreach (var child in children.Items)
			{
				var saveMethod = typeof(DbAccessor).GetMethod(nameof(DbAccessor.Save))!.MakeGenericMethod(children.GenericType);
	...
	public static void Delete<T>(IDbConnection connection, T instance, int Timeout = 30)
	{
		...
		foreach (var idnetifer in def.ChildIdentifers)
		{
			...
			foreach (var child in children.Items)
			{
				var deleteMethod = typeof(DbAccessor).GetMethod(nameof(DbAccessor.Delete))!.MakeGenericMethod(children.GenericType);
