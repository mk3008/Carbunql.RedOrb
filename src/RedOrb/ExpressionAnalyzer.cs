...
{
	internal static IEnumerable<Expression> GetExpressions(this Expression exp)
	{
		yield return exp;
	...
	public static string Analyze(Expression exp)
	{
		...
		else if (exp is BinaryExpression b)
		{
			return Analyze(b);
		}
	...
	internal static string Analyze(BinaryExpression exp)
	{
		var sb = new StringBuilder();

		sb.AppendLine($"* BinaryExpression");
		sb.AppendLine($"NodeType\r\n    {exp.NodeType}");
		sb.AppendLine($"Type\r\n    {exp.Type.Name}");

		sb.AppendLine($"Left");
		sb.AppendLine(Analyze(exp.Left).InsertIndent());

		sb.AppendLine($"Right");
		sb.AppendLine(Analyze(exp.Right).InsertIndent());
